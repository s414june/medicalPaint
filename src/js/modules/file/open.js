...
class File_open_class {
  ...
  open_file() {
    ...
    document.getElementById("file_open").addEventListener(
      "change",
      function (e) {
        _this.open_handler(e);
      },
  ...
  open_dir() {
    ...
    document.getElementById("tmp").appendChild(a);
    document.getElementById("file_open_dir").addEventListener(
      "change",
      function (e) {
        _this.open_handler(e);
      },
  ...
  async open_handler(e) {
    ...
    for (var i = 0, f; i < files.length; i++) {
      ...
      if (f.type == "text/plain") FR.readAsText(f);
      else if (f.name.match(".json")) FR.readAsText(f);
      ...
      item.file(async function (file) {
        ...
        FR.onload = function (event) {
          if (
            ...
            (this.file.type == "" &&
              this.file.name.match(/\.(png|jpg|jpeg|webp|gif|avif)/g))
          ) {
            //image
            var new_layer = {
              name: this.file.name,
  ...
  open_template_test() {
    ...
    window
      .fetch("images/test-collection.json")
