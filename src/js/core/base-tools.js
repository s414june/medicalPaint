...
class Base_tools_class {
    ...
    document.addEventListener(
      ...
      function (event) {
        ...
        if (
          ...
          !$(".scroll").has($(event.target)).length
        )
  ...
  set_mouse_info(event) {
    ...
    if (eventType === "mousedown" || eventType === "touchstart") {
      if (
        (event.target.id != "canvas_minipaint" &&
          event.target.id != "main_wrapper") ||
        (event.which != 1 && eventType !== "touchstart")
      ) {
        this.mouse_click_valid = false;
      } else {
  ...
  get_params_hash() {
    var data = [this.getParams(), config.COLOR, config.ALPHA];
  ...
  shape_mousedown(e) {
    ...
    app.State.do_action(
      new app.Actions.Bundle_action(
        "new_" + this.name + "_layer",
        "New " + this.Helper.ucfirst(this.name) + " Layer",
  ...
  get_snap_positions(exclude_id) {
    var snap_positions = {
      x: [0, config.WIDTH / 2, config.WIDTH],
      y: [0, config.HEIGHT / 2, config.HEIGHT],
    };
    if (config.guides_enabled == true) {
      //use guides
      for (var i in config.guides) {
        var guide = config.guides[i];
        if (guide.y === null) snap_positions.x.push(guide.x);
        else snap_positions.y.push(guide.y);
      }
    }
    for (var i in config.layers) {
      ...
      if (
        ...
        config.layers[i].x === null ||
        config.layers[i].y === null ||
        config.layers[i].width === null ||
        config.layers[i].height === null
      ) {
        continue;
      }
      ...
      if (x > 0 && x < config.WIDTH) snap_positions.x.push(x);
      ...
      if (y > 0 && y < config.HEIGHT) snap_positions.y.push(y);
  ...
  calc_snap_position(event, pos_x, pos_y, exclude_id) {
    ...
    else var snap_positions = this.get_snap_positions();
    ...
    for (var i in snap_positions.x) {
      ...
      if (
        ...
        (distance < min_distance.x || min_distance.x === null)
      ) {
    ...
    for (var i in snap_positions.y) {
      ...
      if (
        distance < max_distance &&
        (distance < min_distance.y || min_distance.y === null)
      ) {
