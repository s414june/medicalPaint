...
class GUI_tools_class {
  ...
  render_tools() {
    ...
    for (var i in config.TOOLS) {
      ...
      if (item.title) var title = item.title;
      else var title = this.Helper.ucfirst(item.name).replace(/_/, " ");
  ...
  show_action_attributes() {
    ...
    for (var k in attributes) {
      ...
      if (
        ...
        selectList.id = k;
        const values =
          typeof item.values === "function" ? item.values() : item.values;
        for (var j in values) {
        ...
        alertify.error(
          "Error: unsupported attribute type:" + typeof item + ", " + k
        );
      }
    }
